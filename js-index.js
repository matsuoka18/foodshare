...
function firstsearch2() {
  fetch('https://script.google.com/macros/s/AKfycbwBH_VrPaXcJg8HOXfoWHJY8f0Ir3935fqlJlURpyAkd8IdEQ/exec', {
    ...
    .then(resJson => {
      ...
      console.log("fc:" + fc);
      if (fc == -1) {
       fc2 = firstdata11.indexOf("shop1");
        if(fc2 >0){
          console.log("Replace→firstdata1 to firstdata2");
          firstdata2ins = 'skip';
          firstdata2 = firstdata11;
        }
...
if(firstdata2n > 0){
console.log("firstmenu2:"+firstdata2);
firstmenuc();
}else{
  order = 1;
  console.log("firstmenu2() → firstmenuall() reload");
  firstmenuall();
}
    })
    .catch(error => {
